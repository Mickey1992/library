...
<html>
  ...
  <center>
    <table style="background:url(image/ab.jpg); "><tr><td>
	   ...
	   <img src="image/head.jpg"  ></td></tr><tr><td><br>
  <center><h1>外借排行</h1> <br><br></center> 
  <form name="rank" id="rank" method="post" action="stu/bookingCheck.jsp">   
   <center> <input name="booking" type="submit" value="预订已选"><br><br><br></center>
    
   <%
   String sql="select top 10 count(s_number) as rent_times,book_ID from rent group by book_ID";
   ResultSet rank =connection.executeQuery(sql);
   int j=1;
   while(rank.next())
   {
   String sql1="select * from BOOK where book_id = '"+rank.getString(2)+"'";
